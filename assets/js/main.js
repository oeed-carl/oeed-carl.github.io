$(document).ready(function() {
			...
			if (success && data) {
				...
				cards = [{
					...
					details: [{
						...
						title: "Transmission",
						value: data.Transmission ? data.Transmission.DisplayValue : undefined
					}, {
						title: "4WD",
				...
				cards = [{
					...
					text: plate + " is not a registered license plate."
				}]
			}
			setTimeout(function() {
	...
	function changeContent(license, cards) {
		...
		for (var i = 0; i < cards.length; i++) {
			...
			var $card
			if (card.isAlert) {
				$card = $(`<div class="card card-alert card-new in"><div class="alert-title">${card.title}</div><div class="alert-text">${card.text}</div><div class="center-buttons"><a href="/"><button>Try Another</button></a></div></div>`).css("animation-delay", (delay + 0.3) + "s")
				$card.find("a").click(function(event) {
					back()
					event.preventDefault()
					return false
				})
				...
				$card = $(`<div class="card card-new in"><div class="title">${card.title}</div></div>`).css("animation-delay", (delay + 0.3) + "s")
